...
async function getAnalytics(userId, months = 6) {
  const { startDate } = getDateRange(months);
...
async function getCategoryTotals(userId, months = 1) {
  const { startDate } = getDateRange(months);
  ...
  transactions.forEach((t) => {
    if (!result[t.category]) {
      result[t.category] = { income: 0, expenses: 0 };
    }

    if (t.type === 'income') {
      result[t.category].income += t.amount;
    } else {
      result[t.category].expenses += t.amount;
    }
  });
...
async function getMonthlyComparison(userId, months = 3) {
  const { startDate } = getDateRange(months);
